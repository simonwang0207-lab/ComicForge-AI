...
当前项目已经形成从故事输入到漫画成品的完整单页闭环，支持离线 Mock、本地 Ollama、本地 ComfyUI、OpenAI-compatible/DeepSeek 文本接口以及多种云端图片 Provider。Recraft、ComfyUI 和 Gemini 已有真实图片生成记录；其他 Provider 的代码适配、连通或真实验收状态在本文中分别标注。
...
本项目的核心目标是把分散的文本创作、剧本审查、角色参考、图片生成、漫画排字和页面导出能力组织为一套连贯的创作工具，降低普通使用者在多个模型平台、脚本和本地工作流之间切换的成本。具体目标包括：

1. 让用户从一个故事主题或已有剧本出发，完成角色设定、叙事节奏和逐格分镜设计。
2. 在图片生成前提供独立审查与人工确认，及时发现人物关系、因果、语言和画面表达问题，减少无效生图费用。
3. 允许用户按质量、速度、成本和设备条件，自由组合本地或云端的文本模型与图片模型。
4. 将图片生成结果进一步加工为具有对白、旁白、拟声词和页面节奏的漫画，而不是只返回互不关联的单张图片。
5. 支持项目保存、重新载入、单格重生成和版本回退，使漫画可以反复修改，而不是一次生成后无法继续编辑。
6. 记录每格画面的模型、提示词、参考角色、耗时和错误，为效果比较、问题定位和模型选型提供依据。
7. 在没有 API Key 或本地大模型时仍可运行离线演示，并为后续新增模型、扩展多页制作和部署保留清晰接口。
...
- 漫画排字、页面布局和导出模块；
- 项目保存、载入、局部重生成和历史回退；
- 自动化测试、严格 Provider smoke test 和按用途组织的正式文档集。
...
flowchart TD
    ...
    TextRegistry --> DeepSeek[DeepSeek]
    ...
    ImageRegistry --> Gemini[Gemini Image]
...
| DeepSeek | 是 | 是 | 已有初稿和审查真实项目记录 | 8–20 格长项目仍需持续统计结构成功率 |
...
| Gemini Image | 是 | 是 | 已完成 | **已完成无参考、单参考和四格真实生成** | 当前云端角色参考候选；第三方网关参数兼容性需逐项确认 |
...
本节按架构、文本、云端图片、ComfyUI、漫画制作和前端工程六类问题汇总。表格中的“成效”来自自动化测试、真实 Provider 输出、项目 JSON 或前端人工验证；没有真实验收的内容不会写成已完成。更完整的问题演进见 [`DEVELOPMENT_HISTORY.md`](DEVELOPMENT_HISTORY.md)。

### 7.1 从 Mock 原型到可替换架构

| 问题或阶段限制 | 原因分析 | 改进方法 | 改进成效 |
|---|---|---|---|
| 原型只有确定性文本模板 | 初期目标是先验证端到端流程，没有真实大模型接口 | 定义 `TextModelProvider`，实现注册表和环境构建；保留 Mock 作为离线基线 | Mock、Ollama、OpenAI-compatible 和 DeepSeek 可通过同一服务接口切换 |
| 原型图片只是 Pillow 文字占位图 | 尚未定义真实图片模型边界 | 定义 `ImageProvider`、请求、结果和能力模型，注册多个图片 Provider | Mock、云端 API 和 ComfyUI 共用逐格生图与排版流程 |
...
| 开发工具网络中断或会话丢失 | 开发会话状态异常，不属于项目代码错误 | 恢复时核对 AGENTS/TASKS/README、Git diff、测试和项目记录；将有效结论归档到统一演进文档 | 开发过程可以从仓库事实恢复，不依赖单一会话记忆 |
...
```text
pytest：249 passed，14 warnings（均为 Gradio 6.0 迁移提示）
Ruff：通过
compileall：通过
应用导入：通过
```
...
- `outputs/20260809_134318_596879_哪吒闹海/project.json`：DeepSeek 初稿/审查与 Gemini 四格真实生成记录，无图片 Mock 回退。
...
- 持续执行完整免费检查，并处理 Gradio 6.0 迁移提示。
