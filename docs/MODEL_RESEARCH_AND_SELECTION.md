# ComicForge AI 文本与图片模型调研及选型

> 调研更新日期：2026-08-09。本文已将网页中的关键规格、接口、价格、限制和适用性直接写入正文，因此导出为 Word 后无需打开网页也能理解主要结论。文末仍保留来源名称和完整网址，便于评审追溯。价格、模型名称、限额和预览模型状态可能变化，实际充值或接入前应重新核对官方页面。
