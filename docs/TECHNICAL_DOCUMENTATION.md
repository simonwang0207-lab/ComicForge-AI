...
- ComfyUI 不会再将第一格已完成的剧情分镜当作角色身份参考；前端显示 Story Bible 角色顺序，并支持批量导入、拖动排序或从剪贴板逐张加入，后端严格按列表位置绑定角色，不依赖文件名。
- 整图 IPAdapter 参考仅发送给匹配角色的单人近景/中景；远景、建立镜头、鸟瞰、环境、群像及多人镜头会旁路参考，避免参考图构图覆盖分镜场景。参考生效时，绘图 Prompt 将上传图片视为该角色外观的权威来源，不再追加模型生成且可能冲突的发型、服装和配色锚点；当前工作流使用 `weight=0.35`、`end_at=0.65` 的中等约束。现有 API Workflow 每格只有一个 IPAdapter 图片入口，多角色同格若要分别锁定身份仍需多 IPAdapter 或区域约束工作流。
...
解析器按以下顺序处理：提取文本 → 去除 Markdown fence → 标准 JSON 解码 → 有限字段别名和类型归一化 → 合并审查稿允许继承的初稿字段 → Pydantic 校验 → 格数、序号和内容语言规则。审查模型若忘记 `project_patch` 外壳、在顶层只返回一个合法的已知分格子集，程序会将其确定性地按 `sequence` 合并到已验证初稿；未知序号、重复序号和超量分格仍按错误拒绝。一般结构失败时向模型发起一次完整修复请求；不会执行模型文本，也不会无依据补造核心故事。对截断、缺字段和枚举错误分别给出可读错误。

如果项目结构已经有效、仅对白、思考或旁白违反 `content_language`，系统改用最小文字补丁：只发送错误分格的 `sequence`、文字 `index`、类型、说话人和原文字，模型只返回新 `text`。文字补丁使用低随机性和较小输出预算，默认最多执行两次；第二次只包含第一次修复后仍不合格的索引。程序按索引合并后再次检查语言和对白质量，角色、说话人、文字类型、场景、动作、布局及英文 `image_prompt` 均保留初稿内容，不再为一次语言错误重写完整项目 JSON。次数可通过 `TEXT_MODEL_LANGUAGE_REPAIR_ATTEMPTS` 调整；设为 `0` 可关闭该专项修复。
